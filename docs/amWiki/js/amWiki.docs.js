...
(function (win, doc, $) {
    ...
    Docs.prototype._setFootnote = function (text) {
        ...
        text = text.replace(noteReg, function (match, s1) {
            for (var i of footnotes) {
                if (i.note == s1) {
                    i.used = true;
                    return templates[0].replace(/{{index}}/g, i + 1 + '').replace('{{title}}', i.title);
                }
            }
            //当脚注的正文不存在，视标记文本为正文
        ...
        if (footnotes.length >= 1) {
            for (var i of footnotes) {
                if (i.used) {
                    ...
                    html += templates[3].replace('{{content}}', i.content);
                }
            }
